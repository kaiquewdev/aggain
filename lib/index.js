...
function _createFileHandler(type, dir, name, fn) {
  ...
  if (type === 'configs') {
    indexAppendData = 'exports.' + name + ' = require(\'./' + name + '\')(env);\n';
  }
  ...
  if (!hasFile) {
    ...
    fileCopyData = mustache.render(file, {
      name: name,
      variableName: formatHandler('variable', name),
      methodName: formatHandler('method', name),
      className: formatHandler('class', name),
      timestamp: timestamp
    });
